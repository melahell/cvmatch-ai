    ...
    return (
        <div className="w-[210mm] h-[297mm] overflow-hidden bg-white text-slate-800 p-6 mx-auto shadow-xl print:shadow-none print:m-0" id="cv-content">
            ...
            <header className="border-b-2 border-slate-800 pb-4 mb-4 break-inside-avoid">
                ...
                <h2 className="text-lg text-blue-700 font-semibold mt-2">
            ...
            <div className="grid grid-cols-3 gap-5 break-inside-avoid">
                            ...
                            {experiences?.map((exp: any, i: number) => (
                                <div key={i} className="break-inside-avoid">
                                    <div className="flex justify-between items-baseline">
                                        <h4 className="font-bold text-sm text-slate-800">{exp.poste}</h4>
                                        <span className="text-xs font-semibold text-slate-500 whitespace-nowrap">
                                            {exp.debut} - {exp.actuel ? "Présent" : exp.fin}
                                        </span>
                                    </div>
                                    <div className="text-xs font-semibold text-blue-700 mb-0.5">{exp.entreprise}</div>
                ...
                <div className="col-span-1 space-y-4">
                    ...
                    <section className="break-inside-avoid">
                        ...
                        {competences?.techniques && (
                            <div className="mb-3">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-1.5">Techniques</h4>
                                <div className="flex flex-wrap gap-1.5">
                                    {competences.techniques.map((skill: string, i: number) => (
                                        <span key={i} className="bg-slate-100 px-1.5 py-0.5 rounded text-xs font-medium text-slate-700">
                                            {skill}
                                        </span>
                                    ))}
                                </div>
                        ...
                        {competences?.soft_skills && (
                            <div className="mb-3">
                                <h4 className="text-xs font-bold text-slate-500 uppercase mb-1.5">Soft Skills</h4>
                    ...
                    <section className="break-inside-avoid">
                        ...
                        <div className="space-y-2">
                            {formations?.map((edu: any, i: number) => (
                                <div key={i}>
                                    <div className="font-bold text-xs text-slate-800">{edu.diplome}</div>
