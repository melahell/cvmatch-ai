 ...
 * Layout: une colonne sobre, ligne d'accent en haut
...
export default function KakunaTemplate({ data, includePhoto = true, dense = false, displayLimits: dl }: TemplateProps) {
    ...
    return (
        <div className="w-[var(--cv-page-width)] min-h-[var(--cv-page-height)] bg-white print:bg-white mx-auto" style={{ fontFamily: "var(--cv-font-body)", color: colors.text }}>
            ...
            <div className={padding}>
                <div className="flex items-start gap-4 py-4 border-b" style={{ borderColor: colors.primary50 }}>
                ...
                <main className="py-4 space-y-4">
                    {profil.elevator_pitch && <p className={`${textSize} text-gray-600 border-l-2 pl-3`} style={{ borderColor: colors.primary }}>{sanitizeText(profil.elevator_pitch)}</p>}
                    {allSkills.length > 0 && (
                        <section><h2 className="text-base font-bold mb-2">Compétences</h2><div className="flex flex-wrap gap-2">{allSkills.map((s: string, i: number) => <span key={i} className={`px-2 py-0.5 rounded ${textSize}`} style={{ backgroundColor: colors.primary20 }}>{sanitizeText(s)}</span>)}</div></section>
                    )}
                    ...
                    {certifications.length > 0 && <section><h2 className="text-base font-bold mb-2">Certifications</h2><ul className={`${textSize} space-y-0.5`}>{certifications.slice(0, limits.maxCertifications).map((c: string, i: number) => <li key={i} className="flex gap-2"><span style={{ color: colors.primary }}>✓</span>{sanitizeText(c)}</li>)}</ul></section>}
                    ...
                    {(clientsReferences?.clients?.length ?? 0) > 0 && <section><h2 className="text-base font-bold mb-2">Clients Références</h2><div className="flex flex-wrap gap-2">{(clientsReferences?.clients ?? []).slice(0, limits.maxClientsReferences).map((c: string, i: number) => <span key={i} className={`px-2 py-0.5 rounded border ${textSize}`} style={{ borderColor: colors.primary }}>{c}</span>)}</div></section>}
                </main>
            </div>
        </div>
    );
