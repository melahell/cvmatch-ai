/**
 * Ivory - RR Amélioré : fond ivoire, accents discrets, très lisible
...
export default function IvoryTemplate({ data, includePhoto = true, dense = false, displayLimits: dl }: TemplateProps) {
    ...
    return (
        <div className="w-[var(--cv-page-width)] min-h-[var(--cv-page-height)] mx-auto print:bg-white" style={{ fontFamily: "var(--cv-font-body)", color: c.text, backgroundColor: "#fefdfb" }}>
            <div className={pad}>
                <header className="py-5 mb-5 rounded-lg px-4" style={{ backgroundColor: c.a10 }}>
                    <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                            <h1 className="text-2xl font-bold" style={{ color: c.text }}>{fullName}</h1>
                ...
                <main className="space-y-5">
                    {profil.elevator_pitch && <p className={`${ts} py-2 px-3 rounded-r-lg border-l-4`} style={{ backgroundColor: c.a10, borderColor: c.p }}>{sanitizeText(profil.elevator_pitch)}</p>}
                    {allSkills.length > 0 && <section><h2 className="text-sm font-bold mb-2" style={{ color: c.p }}>Compétences</h2><div className="flex flex-wrap gap-2">{allSkills.map((s: string, i: number) => <span key={i} className={`${ts} px-2.5 py-1 rounded-full`} style={{ backgroundColor: c.a20 }}>{sanitizeText(s)}</span>)}</div></section>}
                    {experiences.length > 0 && (
                        <section><h2 className="text-sm font-bold mb-3" style={{ color: c.p }}>Expérience</h2><div className="space-y-3">
                            {experiences.map((exp: any, idx: number) => (
                                <article key={idx} className="py-2 pl-3 border-l-2" style={{ borderColor: c.p }}>
                    ...
                    <div className="grid grid-cols-2 gap-5">
                        {formations.length > 0 && <section><h2 className="text-sm font-bold mb-2" style={{ color: c.p }}>Formation</h2>{formations.slice(0, limits.maxFormations).map((f: any, i: number) => <article key={i} className="mb-2"><h3 className="font-bold text-sm">{sanitizeText(f.diplome)}</h3><p className={`${ts} text-gray-600`}>{sanitizeText(f.etablissement)} {f.annee}</p></article>)}</section>}
                        <div className="space-y-4">{langues.length > 0 && <section><h2 className="text-sm font-bold mb-2" style={{ color: c.p }}>Langues</h2>{langues.slice(0, limits.maxLangues).map((l: any, i: number) => <div key={i} className="flex justify-between text-sm mb-1"><span>{l.langue}</span><span className={`${ts} px-2 py-0.5 rounded`} style={{ backgroundColor: c.a20 }}>{l.niveau}</span></div>)}</section>}{certifications.length > 0 && <section><h2 className="text-sm font-bold mb-2" style={{ color: c.p }}>Certifications</h2><ul className={`${ts} space-y-0.5`}>{certifications.slice(0, limits.maxCertifications).map((cert: string, i: number) => <li key={i} className="flex gap-2"><span style={{ color: c.p }}>✓</span>{sanitizeText(cert)}</li>)}</ul></section>}</div>
                    </div>
                    {projects.length > 0 && <section><h2 className="text-sm font-bold mb-2" style={{ color: c.p }}>Projets</h2><div className="grid grid-cols-2 gap-2">{projects.slice(0, limits.maxProjects).map((p: any, i: number) => <article key={i} className="p-2.5 rounded-lg" style={{ backgroundColor: c.a10 }}><h3 className="font-bold text-sm">{sanitizeText(p.nom)}</h3>{p.description && <p className={`${ts} text-gray-600`}>{sanitizeText(p.description)}</p>}</article>)}</div></section>}
                    {(clientsReferences?.clients?.length ?? 0) > 0 && <section><h2 className="text-sm font-bold mb-2" style={{ color: c.p }}>Clients références</h2><div className="flex flex-wrap gap-2">{(clientsReferences?.clients ?? []).slice(0, limits.maxClientsReferences).map((client: string, i: number) => <span key={i} className={`${ts} px-2 py-0.5 rounded border`} style={{ borderColor: c.a50 }}>{client}</span>)}</div></section>}
                </main>
            </div>
        </div>
    );
