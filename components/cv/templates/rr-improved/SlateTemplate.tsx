/**
 * Slate - RR Amélioré : tons slate, séparateurs nets
...
export default function SlateTemplate({ data, includePhoto = true, dense = false, displayLimits: dl }: TemplateProps) {
    ...
    const H = ({ children }: { children: React.ReactNode }) => (
        <h2 className="text-sm font-bold mb-3 flex items-center gap-2">
            <span className="w-1 h-5 rounded-full" style={{ backgroundColor: c.p }} />
            {children}
        </h2>
    );
    ...
    return (
        <div className="w-[var(--cv-page-width)] min-h-[var(--cv-page-height)] bg-white print:bg-white mx-auto" style={{ fontFamily: "var(--cv-font-body)", color: c.text }}>
            <div className={pad}>
                <header className="py-6 mb-6" style={{ borderBottom: `2px solid ${c.a50}` }}>
                ...
                <main className="space-y-6">
                    {profil.elevator_pitch && <section className="py-3" style={{ borderBottom: `1px solid ${c.a50}` }}><p className={`${ts} italic`} style={{ color: c.muted }}>{sanitizeText(profil.elevator_pitch)}</p></section>}
                    {allSkills.length > 0 && <section><H>Compétences</H><div className="flex flex-wrap gap-2">{allSkills.map((s: string, i: number) => <span key={i} className={`px-3 py-1 rounded ${ts}`} style={{ backgroundColor: c.a20 }}>{sanitizeText(s)}</span>)}</div></section>}
                    {experiences.length > 0 && (
                        <section><H>Expérience</H><div className="space-y-4">
