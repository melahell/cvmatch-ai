/**
 * Lumen - RR Amélioré : fond blanc, titres soulignés, très pro
...
export default function LumenTemplate({ data, includePhoto = true, dense = false, displayLimits: dl }: TemplateProps) {
    ...
    const H = ({ children }: { children: React.ReactNode }) => <h2 className="text-sm font-semibold pb-1 mb-3 border-b-2" style={{ borderColor: c.p }}>{children}</h2>;
        ...
        <div className="w-[var(--cv-page-width)] min-h-[var(--cv-page-height)] bg-white print:bg-white mx-auto" style={{ fontFamily: "var(--cv-font-body)", color: c.text }}>
            <div className={pad}>
                <header className="py-6 mb-6">
                    <div className="flex items-start gap-5">
                        ...
                        <div className="flex-1">
                            ...
                            {titre && <p className="text-base mt-0.5 font-medium" style={{ color: c.s }}>{sanitizeText(titre)}</p>}
                ...
                <main className="space-y-5">
                    {profil.elevator_pitch && <p className={`${ts} text-gray-600 leading-relaxed`}>{sanitizeText(profil.elevator_pitch)}</p>}
                    {allSkills.length > 0 && <section><H>Compétences</H><div className="flex flex-wrap gap-2">{allSkills.map((s: string, i: number) => <span key={i} className={`${ts} px-2.5 py-1 rounded`} style={{ backgroundColor: c.a20 }}>{sanitizeText(s)}</span>)}</div></section>}
                    {experiences.length > 0 && <section><H>Expérience</H><div className="space-y-4">{experiences.map((exp: any, idx: number) => (
                    ...
                    <div className="grid grid-cols-2 gap-5">
                        {formations.length > 0 && <section><H>Formation</H>{formations.slice(0, limits.maxFormations).map((f: any, i: number) => <article key={i} className="mb-2"><h3 className="font-bold text-sm">{sanitizeText(f.diplome)}</h3><p className={`${ts} text-gray-600`}>{sanitizeText(f.etablissement)} {f.annee}</p></article>)}</section>}
                        <div className="space-y-4">{langues.length > 0 && <section><H>Langues</H>{langues.slice(0, limits.maxLangues).map((l: any, i: number) => <div key={i} className="flex justify-between text-sm mb-1"><span>{l.langue}</span><span className={`${ts} px-2 py-0.5 rounded`} style={{ backgroundColor: c.a20 }}>{l.niveau}</span></div>)}</section>}{certifications.length > 0 && <section><H>Certifications</H><ul className={`${ts} space-y-0.5`}>{certifications.slice(0, limits.maxCertifications).map((cert: string, i: number) => <li key={i} className="flex gap-2"><span style={{ color: c.p }}>✓</span>{sanitizeText(cert)}</li>)}</ul></section>}</div>
                    </div>
                    {projects.length > 0 && <section><H>Projets</H><div className="grid grid-cols-2 gap-2">{projects.slice(0, limits.maxProjects).map((p: any, i: number) => <article key={i} className="p-2 rounded" style={{ backgroundColor: c.a20 }}><h3 className="font-bold text-sm">{sanitizeText(p.nom)}</h3>{p.description && <p className={`${ts} text-gray-600`}>{sanitizeText(p.description)}</p>}</article>)}</div></section>}
                    {(clientsReferences?.clients?.length ?? 0) > 0 && <section><H>Clients références</H><div className="flex flex-wrap gap-2">{(clientsReferences?.clients ?? []).slice(0, limits.maxClientsReferences).map((client: string, i: number) => <span key={i} className={`${ts} px-2 py-0.5 rounded border`} style={{ borderColor: c.p }}>{client}</span>)}</div></section>}
                </main>
            </div>
        </div>
    );
