/**
 * Nova - RR Amélioré : bandeau header sombre, contenu clair, contraste fort
...
export default function NovaTemplate({ data, includePhoto = true, dense = false, displayLimits: dl }: TemplateProps) {
    ...
    return (
        <div className="w-[var(--cv-page-width)] min-h-[var(--cv-page-height)] bg-white print:bg-white mx-auto" style={{ fontFamily: "var(--cv-font-body)", color: c.text }}>
            <header className={`${pad} py-6 text-white`} style={{ background: `linear-gradient(180deg, ${c.p} 0%, ${c.s} 100%)` }}>
                <div className="flex items-center gap-4">
                    {includePhoto && <ProfilePicture photoUrl={profil.photo_url} fullName={fullName} initials={initials} includePhoto={includePhoto} size="lg" borderColor="#fff" className="flex-shrink-0 ring-2 ring-white/50" />}
                    <div className="flex-1">
                        <h1 className="text-2xl font-bold">{fullName}</h1>
                        {titre && <p className="text-white/90 text-base mt-0.5">{sanitizeText(titre)}</p>}
                        <ContactInfo email={profil.email} telephone={profil.telephone} localisation={profil.localisation} linkedin={profil.linkedin} github={profil.github} portfolio={profil.portfolio} layout="inline" textColor="rgba(255,255,255,0.85)" iconColor="rgba(255,255,255,0.85)" className={`flex flex-wrap gap-3 mt-2 ${ts}`} />
                    </div>
                </div>
            </header>

            <div className={pad}>
                <main className="space-y-5 -mt-2 pt-6 rounded-t-lg bg-white relative" style={{ boxShadow: "0 -2px 10px rgba(0,0,0,0.05)" }}>
                    {profil.elevator_pitch && <p className={`${ts} text-gray-600 italic border-l-4 pl-4 py-1`} style={{ borderColor: c.p }}>{sanitizeText(profil.elevator_pitch)}</p>}
                    {allSkills.length > 0 && <section><h2 className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: c.p }}>Compétences</h2><div className="flex flex-wrap gap-2">{allSkills.map((s: string, i: number) => <span key={i} className={`${ts} px-2.5 py-1 rounded`} style={{ backgroundColor: c.a20 }}>{sanitizeText(s)}</span>)}</div></section>}
                    ...
                    {(clientsReferences?.clients?.length ?? 0) > 0 && <section><h2 className="text-xs font-bold uppercase tracking-wider mb-2" style={{ color: c.p }}>Clients références</h2><div className="flex flex-wrap gap-2">{(clientsReferences?.clients ?? []).slice(0, limits.maxClientsReferences).map((client: string, i: number) => <span key={i} className={`${ts} px-2 py-0.5 rounded border`} style={{ borderColor: c.p }}>{client}</span>)}</div></section>}
                </main>
            </div>
        </div>
    );
