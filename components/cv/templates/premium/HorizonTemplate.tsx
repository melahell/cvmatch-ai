...
/**
 * HORIZON — Minimaliste spacieux, gradient accent stripe left
 * 5mm stripe gradient sur la gauche. Header horizontal avec photo + infos.
 * Single-column body compact. Sections secondaires groupées en flex-row.
 * Cible : Architecture, Urbanisme, Design graphique, Profils créatifs épurés.
 */
    ...
    return (
        ...
        >
            <div className="flex h-full">
                {/* ── LEFT ACCENT STRIPE (5mm) ── */}
                <div
                    className="w-[5mm] flex-shrink-0"
                    style={{
                        background: `linear-gradient(180deg, ${V.primary} 0%, ${V.sidebarBg} 50%, ${V.primary} 100%)`,
                    }}
                />
                ...
                <div className="flex-1 flex flex-col">
                    {/* ── HEADER ── */}
                    <header className="flex items-center gap-5 px-6 pt-5 pb-3" style={{ borderBottom: `1px solid ${V.primaryA20}` }}>
                        {includePhoto && (
                            <ProfilePicture
                        ...
                        <div className="flex-1">
                            <h1 className="text-[20pt] font-bold text-gray-900 tracking-tight">{fullName}</h1>
                            <p className="text-[10pt] font-medium" style={{ color: V.primary }}>{profil?.titre_principal}</p>
                            ...
                            {jobContext?.job_title && (
                                <div
                                    className="mt-1.5 text-[7pt] px-2 py-0.5 rounded inline-block"
                    ...
                    {/* ── BODY ── */}
                    <main className="px-6 pt-3 pb-5 flex-1">
                        {/* Pitch */}
                        ...
                        {/* Compétences — en bloc teinté côte-à-côte */}
                        {(limitedSkills.length > 0 || limitedSoftSkills.length > 0) && (
                            <section className={sectionMb}>
                                <SectionTitle title="Compétences" primaryColor={V.primary} variant="accent-line" textSize="text-[9.5pt]" />
                                <div className="flex gap-4">
                                    {limitedSkills.length > 0 && (
                                        <div className="flex-1">
                                            <p className="text-[7pt] font-semibold text-gray-400 uppercase tracking-wider mb-1">Techniques</p>
                                            <SkillsGrid
                                    ...
                                    {limitedSoftSkills.length > 0 && (
                                        <div className="flex-1">
                                            <p className="text-[7pt] font-semibold text-gray-400 uppercase tracking-wider mb-1">Savoir-être</p>
                                            <SkillsGrid
                        ...
                        {/* Formation + Langues + Certifications — côte-à-côte */}
                        {(limitedFormations.length > 0 || limitedLangages.length > 0 || limitedCertifications.length > 0) && (
                            <section className={sectionMb}>
                                <div className="flex gap-4">
                                    {limitedFormations.length > 0 && (
                                        <div className="flex-1 min-w-[30%]">
                                            <SectionTitle title="Formation" primaryColor={V.primary} variant="accent-line" textSize="text-[9pt]" />
                                    ...
                                    {limitedLangages.length > 0 && (
                                        <div className="flex-1 min-w-[20%]">
                                            <SectionTitle title="Langues" primaryColor={V.primary} variant="accent-line" textSize="text-[9pt]" />
                                    ...
                                    {limitedCertifications.length > 0 && (
                                        <div className="flex-1 min-w-[20%]">
                                            <SectionTitle title="Certifications" primaryColor={V.primary} variant="accent-line" textSize="text-[9pt]" />
